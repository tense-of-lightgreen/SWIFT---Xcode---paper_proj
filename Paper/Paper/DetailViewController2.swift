...
class DetailViewController2 : UIViewController, UIWebViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        NSLog("linkurl = \(self.mvo?.author), title=\(self.mvo?.title)")
        
        // 내비게이션 바의 타이틀에 영화명을 출력한다.
        let navibar = self.navigationItem
        navibar.title = self.mvo?.title
        print("selfmvo ======== \(self.mvo)")
        if let url = self.mvo?.author {
